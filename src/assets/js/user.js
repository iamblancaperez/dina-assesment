function User(firstName = "", lastName = "", status = ""){
	...
	this.renderForm = function(container, action = "create"){
		...
		form.addEventListener("submit", function(event) {
	  	...
	  	var firstName = form.querySelector("#first-name");
			...
			if(response.success){
				if(action == "create"){
					serverResponse = createUser({"first_name" : realThis.firstName, "last_name" : realThis.lastName, "status" : realThis.status});
				}else if(action == "edit"){
					serverResponse = updateUser(363, {"first_name" : realThis.firstName, "last_name" : realThis.lastName, "status" : realThis.status});
				}
			}else{
	...
	this.formTemplate = `<div class="form-container">
